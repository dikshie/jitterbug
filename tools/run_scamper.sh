...
OUTPUT_ANALYSIS="analysis_results.json"
IP_VERSION=""

usage() {
  cat <<EOF
Usage: $0 [options] [TARGET] [COUNT] [OUTPUT_JSON] [OUTPUT_ANALYSIS]
...
Options:
  ...
  -a, --analysis FILE  Output analysis results file (default: analysis_results.json)
  -h, --help           Show this help message

Examples:
  $0 -4 ns4.indosat.com 120 scamper_ns4.json analysis.json
  $0 -6 2001:4860:4860::8888 60 output.json analysis.json
  $0 --ipv4 --target ns4.indosat.com --count 120
EOF
  exit 0
}
...
if [[ ${#positional_args[@]} -ge 4 ]]; then
  OUTPUT_ANALYSIS="${positional_args[3]}"
fi
...
echo "=== Running Scamper Ping for target: ${TARGET} (${RESOLVED_TARGET}) ==="
echo "=== IP Version: ${IP_LABEL} | Probes: ${COUNT} ==="
sudo scamper -O json -o "${OUTPUT_JSON}" -c "ping -c ${COUNT} -i 1" -i "${RESOLVED_TARGET}"
